...
def mock_get_date_for_dowanlad() -> pd.DatetimeIndex:
    return pd.date_range(start="20201231", end="20211231", freq="Y")


def mock_get_merged_table_by(stock_code) -> pd.DataFrame:
    data: dict[str:Any] = {
        C_NET_PROFIT: [50, 45],
        C_EQUITY: [100, 92],
...
def prepare_mock(monkeypatch) -> None:
    monkeypatch.setattr(yu, "get_merged_table_by", mock_get_merged_table_by)
...
class TestRoeAnalyzer:
    ...
    def test_voting(self, monkeypatch):
        ...
        roe_a: RoeAnalyzer = RoeAnalyzer("600519")
        voted: pd.Series = roe_a.voting()
    ...
    def test_analyzing(self, monkeypatch):
        ...
        roe_a: RoeAnalyzer = RoeAnalyzer("600519")
        df: pd.DataFrame = roe_a.analyzing()
        assert not df.empty
        assert roe_a._is_analyzed
        assert round(df.loc[0, C_ROE], 2) == 0.5

    def test_do_analyzing(self, monkeypatch) -> None:
        prepare_mock(monkeypatch)

        roe_a: RoeAnalyzer = RoeAnalyzer("600519")
        p_df: pd.DataFrame = roe_a._preprocess()
        a_df: pd.DataFrame = roe_a._do_analyzing(p_df)
...
class TestFocAnalyzer:
    def test__do_analyzing(self, monkeypatch):
        ...
        foc_a: FocAnalyzer = FocAnalyzer("600519")
        p_df: pd.DataFrame = foc_a._preprocess()
        assert not p_df.empty
        assert not p_df[C_OPER_NET_CASH].empty
        assert not p_df[C_CAPITAL_COST].empty

        a_df: pd.DataFrame = foc_a._do_analyzing(p_df)
...
class TestOpmAnalyzer:
    def test__get_voting_columns(self, monkeypatch):
        ...
        opm: OpmAnalyzer = OpmAnalyzer("600519")
    ...
    def test__do_analyzing(self, monkeypatch):
        prepare_mock(monkeypatch)
        opm: OpmAnalyzer = OpmAnalyzer("600519")
        a_df: pd.DataFrame = opm.analyzing()
        assert not a_df.empty
