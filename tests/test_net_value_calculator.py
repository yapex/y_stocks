...
class TestNetValueCalculator:
    def test_add_first(self) -> None:
        ...
        df: pd.DataFrame = nv_c.take_snapshot()
        assert len(df) == 1
        assert df["净值"][0] == 1.0000

    def test_add_more(self) -> None:
        nv_c: NetValueCalculator = NetValueCalculator()
        nv_c.record_invest(date_time="2018-01-01",
                           invest_value=0, add_cash=6494005.25)
        nv_c.record_invest(date_time="2018-01-02", invest_value=6595041.35)
        nv_c.record_invest(
            date_time="2018-01-03", invest_value=6748813.81, add_cash=71612.17
        )
